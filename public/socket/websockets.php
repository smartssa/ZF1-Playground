...
abstract class WebSocketServer implements SplSubject {
		...
		while(true) {
			...
			foreach ($read as $socket) {
				if ($socket == $this->master) {
					...
					if ($numBytes == 0) {
						$this->stdout('disconnected, zero bytes');
						$this->disconnect($socket);
					} else {
						$user = $this->getUserBySocket($socket);
						if (!$user->handshake) {
							$this->stdout('handshaking');
							$this->doHandshake($user,$buffer);
						} else {
							if ($message = $this->deframe($buffer,$user)) {
								$this->stdout('processing message');
	...
	protected function doHandshake($user, $buffer) {
		...
		if (!isset($headers['connection']) || strpos(strtolower($headers['connection']), 'upgrade') === FALSE) {
			$this->stderr('bad request, no connection header');
			$handshakeResponse = "HTTP/1.1 400 Bad Request";
		}
		if (!isset($headers['sec-websocket-key'])) {
			$this->stderr('bad request, no websocket-key');
			$handshakeResponse = "HTTP/1.1 400 Bad Request";
		} else {
			
		}
		if (!isset($headers['sec-websocket-version'])) {
//			$this->stdout('Browser claims: ' . $headers['sec-websocket-version']);
//			$handshakeResponse = "HTTP/1.1 426 Upgrade Required\r\nSec-WebSocketVersion: 13";
		...
		if (isset($handshakeResponse)) {
			$this->stderr('disconnecting due to handshake failure: ' . $handshakeResponse);
	...
	public function detach(SplObserver $objserver) {}
	public function notify() {
		foreach ($this->users as $observer) {
